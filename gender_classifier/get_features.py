...
def get_feats(df, auds_dir, feats_dir, fem_flag):
...
def main(in_file, auds_dir, out_file, feats_dir):
    ...
    # Contributions by females
    females_df = filedf[filedf['Gender'].fillna(0) == 2]

    # Contributions by males
    males_df = filedf[filedf['Gender'].fillna(0) == 1]

    start_time = time.time()
    fem_feats = get_feats(females_df, auds_dir, feats_dir, fem_flag=True)
    end_time = time.time()

    mal_feats = get_feats(males_df, auds_dir, feats_dir, fem_flag=False)
    
    # the combined acc and rej feats
    fem_feats.extend(mal_feats)         # fem feats now contains all the feats (fem+mal)
