...
if args.start_date and args.end_date is not None:
    ...
    news_blank = News.objects.filter(time__gte = start_datetime, time__lt = end_datetime, state__in = ['REJ'], tags__icontains='blank', source__in=[1,2])
    news_accept = News.objects.filter(time__gte = start_datetime, time__lt = end_datetime, state__in = ['ARC', 'PUB'], source__in=[1,2], rating__in=[4,5])
    #news = News.objects.filter(time__gte = start_datetime, time__lt = end_datetime, state__in = ['ARC', 'PUB'])

    if news_blank.count() < int(args.thresh):
        print("Lesser than threshold number of items in Blank class, please increase the dates because there are {} items".format(news_blank.count()))
        sys.exit() 
    if news_accept.count() < int(args.thresh):
...
try:
    ...
    generate_attachment(data_accept, output_file)
    send_email('Tag Data', 'Tag Data', email_ids, output_file)
except Exception as e:
